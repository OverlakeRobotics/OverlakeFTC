...
public abstract class BaseOpMode extends OpMode {
    ...
    public void init() {
        ...
        imuSystem = new IMUSystem(hardwareMap.get(BNO055IMU.class, DriveParams.IMU));
    }
    ...
    private void setDriveSystem() {
        ...
        driveSystem = new DriveSystem(driveMap, imuSystem);
    }
    ...
    protected boolean align(int colorSignature, int desiredWidth) {
        if (Math.abs(imuSystem.getHeading()) >= 90) {
            if(bail()){
                bail = true;
                return true;
            }
...
//        }
    ...
    public boolean bail() {
        if (bStep == 0) {
           if(driveSystem.turnAbsolute(0, 0.3)){
                bStep = 0;
                return true;
            }
        }
